...
export default function CategoriesPage() {
  ...
  async function remove() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Delete failed. Make sure no active products use this category.");
    }
  }
    ...
    <AppShell>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Organize product master data for search and reporting.</p>
        </div>
      ...
      <div>
        <section className="overflow-hidden rounded-md border border-line bg-white">
          {categories.length === 0 ? <div className="p-5 text-sm text-slate-500">No categories yet.</div> : null}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 px-4 py-6 backdrop-blur-sm">
          <div className="max-h-[calc(100vh-48px)] w-full max-w-lg overflow-y-auto rounded-md border border-line bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {form.id ? "Update category details used by products and reports." : "Create a category for product organization."}
                </p>
              </div>
              <button className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-line text-slate-600 hover:bg-field" onClick={closeForm} aria-label="Close category form">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-3">
              ...
              <label className="block text-sm font-semibold">Description<Input className="mt-1" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>
            </div>
            <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
              <div>
                {form.id ? <Button className="w-full bg-red-600 hover:bg-red-700 sm:w-auto" onClick={remove}><Trash2 className="h-4 w-4" />Delete</Button> : null}
              </div>
      ...
      <ConfirmModal
        ...
        description="This will update the category used by product master data."
        confirmLabel="Update Category"
        onCancel={() => setConfirmUpdateOpen(false)}
        onConfirm={save}
      >
        <div className="font-semibold">{form.name || "Selected category"}</div>
