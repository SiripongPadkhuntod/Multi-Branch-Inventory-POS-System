...
export default function TransfersPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Cannot load transfer data");
    }
  }
  ...
  useEffect(() => {
    if (fromBranchId) {
      api.inventories("", fromBranchId)
        ...
        .catch((err) => setError(err instanceof Error ? err.message : "Cannot load source stock"));
    }
  }, [fromBranchId]);
  ...
  async function transfer() {
    ...
    if (!fromBranchId || !toBranchId || !productId || quantity < 1) {
      setError("Source branch, destination branch, product, and quantity are required");
      return;
    }
  ...
  return (
    <AppShell>
      <div className="mb-5">
        ...
        <p className="text-sm text-slate-500">Move stock between branches with inventory movement logs.</p>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_380px]">
        <section className="overflow-hidden rounded-md border border-line bg-white">
          <div className="border-b border-line p-4 font-semibold">Source Branch Stock</div>
          {sourceStock.length === 0 ? <div className="p-5 text-sm text-slate-500">No stock in source branch.</div> : null}
        ...
        <aside className="rounded-md border border-line bg-white p-4">
          <h2 className="mb-4 flex items-center gap-2 text-lg font-bold"><ArrowRightLeft className="h-5 w-5 text-brand" />New Transfer</h2>
          <div className="space-y-3">
            <label className="block text-sm font-semibold">From Branch
            ...
            <label className="block text-sm font-semibold">To Branch
            ...
            <label className="block text-sm font-semibold">Quantity<Input className="mt-1" type="number" min={1} value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} /></label>
            <Button className="w-full" onClick={transfer}><Save className="h-4 w-4" />Transfer Stock</Button>
          </div>
        </aside>
      </div>
    </AppShell>
