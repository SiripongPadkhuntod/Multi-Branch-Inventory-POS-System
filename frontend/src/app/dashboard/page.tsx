...
export default function DashboardPage() {
  ...
  const widgets = [
    { label: "Daily Sales", value: summary.daily_sales.toLocaleString(), icon: ReceiptText },
    { label: "Monthly Sales", value: summary.monthly_sales.toLocaleString(), icon: BarChart3 },
    { label: "Revenue", value: money(summary.revenue), icon: TrendingUp },
    { label: "Profit", value: money(summary.profit), icon: PackageSearch }
  ];

  const salesTrend = useMemo(() => buildSalesTrend(sales), [sales]);
    ...
    <AppShell>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {selectedBranch ? `${selectedBranch.code} · ${selectedBranch.name}` : "Sales and stock summary across accessible branches."}
          </p>
        </div>
        <select
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-md border border-line bg-white p-4">
          ...
          <div className="mt-3 space-y-2">
            {summary.branch_comparison.length === 0 ? <div className="text-sm text-slate-500">No branch data.</div> : null}
...
function buildSalesTrend(sales: Sale[]): TrendPoint[] {
...
function RevenueTrendChart({ data }: { data: TrendPoint[] }) {
  ...
  const area = `32,178 ${line} ${points[points.length - 1]?.x ?? 416},178`;
  ...
  return (
    <article className="rounded-md border border-line bg-white p-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="font-semibold">7-Day Revenue Trend</h2>
          <p className="text-sm text-slate-500">Revenue from visible branch sales.</p>
        </div>
        <div className="text-sm text-slate-500 sm:text-right">
          <div className="font-semibold text-slate-900">{compactMoney(totalRevenue)}</div>
          <div>{totalSales} sale(s)</div>
        </div>
      </div>
      <div className="mt-4 overflow-x-auto">
        <svg viewBox="0 0 448 220" className="h-64 min-w-[448px] w-full" role="img" aria-label="Seven day revenue trend chart">
          <defs>
          ...
          <polyline points={line} fill="none" stroke="#2f7f75" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
          {points.map((point) => (
            <g key={point.key}>
...
function RevenueDonut({ revenue, profit }: { revenue: number; profit: number }) {
  ...
  return (
    <article className="rounded-md border border-line bg-white p-4">
      ...
      <p className="text-sm text-slate-500">Profit against gross revenue.</p>
      <div className="mt-5 flex flex-col items-center gap-5 sm:flex-row">
        ...
        <div className="w-full space-y-3 text-sm">
          <div className="flex items-center justify-between rounded-md bg-field p-3">
            <span className="flex items-center gap-2"><span className="h-3 w-3 rounded-sm bg-brand" />Profit</span>
            <span className="font-semibold">{money(profit)}</span>
          </div>
          <div className="flex items-center justify-between rounded-md bg-field p-3">
            <span className="flex items-center gap-2"><span className="h-3 w-3 rounded-sm bg-slate-800" />Cost share</span>
...
function TopProductsChart({ products }: { products: DashboardSummary["top_products"] }) {
  ...
  return (
    <article className="rounded-md border border-line bg-white p-4">
      <h2 className="font-semibold">Top Products Chart</h2>
      <p className="text-sm text-slate-500">Ranked by revenue from sold items.</p>
      <div className="mt-4 space-y-3">
        {products.length === 0 ? <div className="rounded-md bg-field p-4 text-sm text-slate-500">No product sales yet.</div> : null}
...
function BranchComparisonChart({ branches }: { branches: DashboardSummary["branch_comparison"] }) {
  ...
  return (
    <article className="rounded-md border border-line bg-white p-4">
      <h2 className="font-semibold">Branch Revenue</h2>
      <p className="text-sm text-slate-500">Compare sales between accessible branches.</p>
      <div className="mt-4 flex h-56 items-end gap-3 overflow-x-auto border-b border-line pb-2">
        {branches.length === 0 ? <div className="self-center text-sm text-slate-500">No branch data.</div> : null}
        {branches.map((branch) => {
          const height = Math.max(12, (branch.revenue / maxRevenue) * 180);
          return (
            <div key={branch.branch_id} className="flex min-w-20 flex-1 flex-col items-center gap-2 text-center">
              <div className="text-xs font-semibold text-slate-700">{compactMoney(branch.revenue)}</div>
...
function LowStockChart({ items }: { items: DashboardSummary["low_stock"] }) {
  ...
  return (
    <article className="rounded-md border border-line bg-white p-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="font-semibold">Low Stock Pressure</h2>
          <p className="text-sm text-slate-500">Items at or below reorder threshold.</p>
        </div>
        <div className="text-sm font-semibold text-slate-700">{items.length} item(s)</div>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {shownItems.length === 0 ? <div className="rounded-md bg-field p-4 text-sm text-slate-500">No low stock items.</div> : null}
        {shownItems.map((item) => (
