    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 px-4 py-6 backdrop-blur-sm">
      <div className="max-h-[calc(100vh-48px)] w-full max-w-md overflow-y-auto rounded-md border border-line bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-line p-4">
          ...
          <button className="grid h-9 w-9 place-items-center rounded-md border border-line text-slate-600 hover:bg-field" onClick={onClose} aria-label={t("common.close")}>
            <X className="h-4 w-4" />
          </button>
        </div>
        <ReceiptPaper detail={detail} />
        <div className="flex flex-col-reverse gap-2 border-t border-line p-4 sm:flex-row sm:justify-end">
          <Button className="!bg-white !text-slate-700 ring-1 ring-line hover:!bg-field" onClick={onClose}>{t("common.close")}</Button>
  ...
  return (
    <div className="bg-white p-5 font-mono text-sm text-slate-950">
      <div className="text-center">
        ...
        <div className="text-xs">{t("receipt.title")}: {detail.receipt_number}</div>
        <div className="text-xs">{t("role.employee")}: {detail.employee_name}</div>
      </div>
          ...
          <div key={item.id}>
            ...
            {item.discount_amount > 0 ? (
              <div className="flex justify-between gap-3 text-xs text-slate-500">
                <span>{t("field.discount")} {item.discount_reason ? `(${item.discount_reason})` : ""}</span>
      ...
      <div className="space-y-1">
        <ReceiptRow label={t("field.subtotal")} value={money(detail.subtotal)} />
        <ReceiptRow label={t("field.discount")} value={money(detail.discount)} />
        <ReceiptRow label={t("sales.tax")} value={money(detail.tax)} />
      ...
      <div className="mt-5 text-center text-xs">{t("receipt.thankYou")}</div>
    </div>
  );
}
...
function receiptHtml(detail: SaleDetail) {
  ...
  const itemRows = detail.items.map((item) => `
    <div class="item">
      ...
      ${item.discount_amount > 0 ? `<div class="row muted"><span>${escapeHtml(t("field.discount"))} ${item.discount_reason ? `(${escapeHtml(item.discount_reason)})` : ""}</span><span>-${money(item.discount_amount)}</span></div>` : ""}
    </div>
  `).join("");
    ...
    <body>
      <main class="receipt">
        <div class="center">
          ...
          <div class="small">${escapeHtml(t("receipt.title"))}: ${escapeHtml(detail.receipt_number)}</div>
          <div class="small">${escapeHtml(t("role.employee"))}: ${escapeHtml(detail.employee_name)}</div>
        </div>
        <div class="dash"></div>
        ${itemRows}
        <div class="dash"></div>
        <div class="row"><span>${escapeHtml(t("field.subtotal"))}</span><span>${money(detail.subtotal)}</span></div>
        <div class="row"><span>${escapeHtml(t("field.discount"))}</span><span>${money(detail.discount)}</span></div>
        <div class="row"><span>${escapeHtml(t("sales.tax"))}</span><span>${money(detail.tax)}</span></div>
        <div class="row total"><span>${escapeHtml(t("field.total"))}</span><span>${money(detail.total)}</span></div>
        <div class="dash"></div>
        ${payments}
        <div class="center small" style="margin-top:18px;">${escapeHtml(t("receipt.thankYou"))}</div>
      </main>
...
function receiptTranslator() {
  const language = (typeof window !== "undefined" && localStorage.getItem("language") === "th" ? "th" : "en") as Language;
  const languageMap = translations[language] as Record<string, string>;
  const fallbackMap = translations.en as Record<string, string>;
  return (key: string) => languageMap[key] ?? fallbackMap[key] ?? key;
}
